<div class="space-y-6">
    ...
    @if($dependents->isEmpty() && !$showForm)
        <div class="bg-gray-50 p-4 rounded-md">
            <p class="text-sm text-gray-700">{{ __('khairat.no_family_members') }}</p>
        </div>
    @endif
                    ...
                    <li class="px-4 py-4 sm:px-6">
                        <div class="flex items-center justify-between">
                            <div class="flex flex-col">
                                ...
                                <p class="mt-1 text-xs text-gray-500">{{ __('khairat.relationship') }}: {{ $dependent->relationship }}</p>
                                @if($dependent->identification_number)
                                    <p class="mt-1 text-xs text-gray-500">{{ __('khairat.identification') }}: {{ $dependent->identification_number }}</p>
                                @endif
                                @if($dependent->birth_date)
                                    <p class="mt-1 text-xs text-gray-500">{{ __('khairat.birth_date') }}: {{ $dependent->birth_date->format('d/m/Y') }}</p>
                                @endif
                            </div>
                            <div class="flex space-x-2">
                                ...
                                <button 
                                    ...
                                    wire:confirm="{{ __('khairat.confirm_delete_dependent') }}"
