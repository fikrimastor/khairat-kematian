<div class="bg-white rounded-lg shadow-md overflow-hidden">
    <div class="p-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-800">{{ __('khairat.receipt') }} #{{ $receipt->receipt_number }}</h2>
            <div class="flex space-x-2">
                ...
                <button wire:click="toggleDetails" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 active:bg-gray-400 focus:outline-none focus:border-gray-400 focus:ring focus:ring-gray-200 disabled:opacity-25 transition">
                    {{ $showDetails ? __('khairat.hide_details') : __('khairat.show_details') }}
                </button>
            </div>
        </div>
        ...
        <div class="border-t border-gray-200 pt-4">
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <p class="text-sm font-medium text-gray-500">{{ __('khairat.receipt_date') }}</p>
                    <p class="mt-1 text-sm text-gray-900">{{ $receipt->formattedDate }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">{{ __('khairat.payment_method') }}</p>
                    <p class="mt-1 text-sm text-gray-900">{{ $payment->payment_method }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">{{ __('khairat.amount') }}</p>
                    <p class="mt-1 text-sm text-gray-900">RM {{ number_format($payment->amount, 2) }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">{{ __('khairat.payment_year') }}</p>
        ...
        @if($showDetails)
            <div class="mt-6 border-t border-gray-200 pt-4">
                <h3 class="text-lg font-medium text-gray-900">{{ __('khairat.member_details') }}</h3>
                <div class="mt-2 grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.name') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $user->name }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.id_number') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $user->identification_number ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.email') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $user->email }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.phone') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $user->phone ?? '-' }}</p>
                    </div>
                </div>
                
                <h3 class="mt-6 text-lg font-medium text-gray-900">{{ __('khairat.payment_details') }}</h3>
                <div class="mt-2 grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.payment_id') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $payment->id }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.reference_number') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $payment->reference_no ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.household_count') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $payment->household_count }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">{{ __('khairat.status') }}</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $payment->status }}</p>
                    </div>
                    @if($payment->notes)
                        <div class="col-span-2">
                            <p class="text-sm font-medium text-gray-500">{{ __('khairat.notes') }}</p>
